...
from datasets.FeedDataset import FeedImageDataset
from datasets.CUHK03.CUHK03 import CUHK03Dataset
...
def load_dataset(config, subset, session, coordinator):
  ...
  if name == "davis":
    ...
    return FeedImageDataset(config=config, subset=subset, num_classes=2, void_label=-1,image_size=(854,480))
  elif name in ("cuhk", "cuhk03"):
    return CUHK03Dataset(config, subset, coordinator)
