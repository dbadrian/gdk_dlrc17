...
import Constants
from Log import log
from Utilf import average_gradients
...
class Trainer(object):
  ...
  def validation_step(self, _):
    yy = self.test_network.y_softmax
    yy_argmax = tf.argmax(yy, 3)
    ops = [self.test_network.loss_summed, self.test_network.measures_accumulated, self.test_network.n_imgs, yy_argmax,self.test_network.tags]
    ...
    # if len(res) > 4:
    loss_summed, measures_accumulated, n_imgs, y, tag = res
      # measures_accumulated[Constants.CLICKS] = tags
    # else:
    #   loss_summed, measures_accumulated, n_imgs,y = res

    ################### JONO ATTEMPT TO SAVE #################
    # folder = '/home/dlrc/Documents/Segment/OnAVOSold/Jono_output/'
    # out_fn = folder + tag[0].decode('utf-8').split("/")[-1].replace(".jpg", ".png").replace(".bin", ".png")
    # # y_argmax = tf.argmax(y, 3)
    # y_scaled = (y * 255).astype("uint8")
    # imsave(out_fn, numpy.squeeze(y_scaled, axis=0))
