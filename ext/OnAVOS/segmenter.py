...
def create_engine():
  ...
  engine = Engine(config,notlatent=True)
  return engine

def create_small_engine():
  # assert len(sys.argv) == 2, "usage: main.py <config>"
  # config_path = sys.argv[1]
  config_path = "configs/jono_small_test"
  assert os.path.exists(config_path), config_path
  try:
    config = Config(config_path)
  except ValueError as e:
...
def create_small_engine2():
  # assert len(sys.argv) == 2, "usage: main.py <config>"
  # config_path = sys.argv[1]
  config_path = "configs/jono_small_test_2"
  assert os.path.exists(config_path), config_path
  try:
    config = Config(config_path)
  except ValueError as e:
